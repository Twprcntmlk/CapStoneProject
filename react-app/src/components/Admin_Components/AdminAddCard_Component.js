...
const AdminAddCard = () => {
  ...
  const onCardAdd= async (e) => {
    e.preventDefault();
    const data = await dispatch(addCard(api_id,api_name,api_set_name,api_set_code,api_set_rarity,api_set_price));
    if (data) {
  ...
  return (
    <div>
      <form onSubmit={onCardAdd} className='CardAddForm'>
      ...
      <div className='cardadd_div'>
        ...
        <div className="addCard">
          <button className="server-button" type="submit">Add Card To Database</button>
        </div>
      </form>
    </div>
  )
